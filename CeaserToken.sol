...
abstract contract Tokenomics {
    ...
    function _getValues(uint256 tAmount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(REDISTRIBUTION_FEE).div(FEES_DIVISOR);
        uint256 tBurn = tAmount.mul(BURN_FEE).div(FEES_DIVISOR);
    ...
    function _getCurrentSupply() internal view returns (uint256, uint256) {
        ...
        if (rSupply < _reflectedSupply.div(TOTAL_SUPPLY)) return (_reflectedSupply, TOTAL_SUPPLY);
    ...
    function _burnTokens(uint256 tBurn, uint256 currentRate) internal {
        uint256 rBurn = tBurn.mul(currentRate);
        _reflectedBalances[BURN_ADDRESS] = _reflectedBalances[BURN_ADDRESS].add(rBurn);
...
contract CeaserToken is Context, IERC20, IERC20Metadata, Tokenomics, Ownable {
    ...
    constructor() {
        ...
        renounceOwnership(); // Renounce ownership to make the contract immutable
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_isExcludedFromRewards[sender]) _balances[sender] = _balances[sender].sub(amount);
